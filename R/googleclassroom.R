...
classroomService <- function(){
  ...
  list(
    ...
    delete_aStudent=delete_aStudent
  )
...
classroom_tokenFetch <- function(email = gargle::gargle_oauth_email(),
  ...
  cred <- gargle::token_fetch(
    ...
    app = httr::oauth_app(
      appname = "classroom",
      key = '35055411830-kqgh2lpfadh29u3dhrvcs6r5hs5jgfec.apps.googleusercontent.com',
      secret = "wKLF5Bgokm3xOZSNcS9C_Rsm"),
    email = email,
    path = path,
    package = "googleclassroom",
    cache = cache,
...
delete_aStudent <- function(courseId, userId){
  requestPhrase <- glue::glue("DELETE https://classroom.googleapis.com/v1/courses/{courseId}/students/{userId}")
  delete_aStudentFun <- classroom_apiFunctional(requestPhrase)
  delete_aStudentFun()
}
